...
class Checker:
    ...
    ██╔╝ ██╗╚██████╗██║  ██║███████╗╚██████╗██║  ██╗███████╗██║  ██║
    ╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝{self.white}
    script © 2023 ~ script#0928\n''', '__________'*10, "\n\n")

        self.thread_starter()

    def title(self):
        while True:
            os.system(f'title script X TikTok Checker ^| Hits ~ {self.hits} : Fails ~ {self.fails} : Rates ~ {self.rates} : Threads ~ {threading.active_count()}' if os.name == 'nt' else '')
